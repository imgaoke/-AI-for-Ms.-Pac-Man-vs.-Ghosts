...
public class Ucs<S, A> {
  static class Node<S, A> implements Comparable<Node<S, A>>{
    ...
    Node<S, A> parent;
    double pathCost;
    Node(S state, A action, double pathCost, Node<S, A> parent){
      this.state = state;
      this.action = action;
    ...
    public int compareTo (Node<S, A> n){
      if (n.pathCost > pathCost){
        return -1;
      }
      else if (n.pathCost == pathCost) {
        return 0;
      }
      else{
        return 1;
  ...
  public static <S, A> Solution<S, A> search(Problem<S, A> prob) {
    ...
    var node = new Node<S, A>(prob.initialState(), null, 0, null);
    ...
    while (!frontier.isEmpty()) {
      ...
      for (var nextAction : prob.actions(nodeBeingExpanded.state)){
        var child = new Node<S, A>(prob.result(nodeBeingExpanded.state, nextAction), nextAction, nodeBeingExpanded.pathCost + prob.cost(nodeBeingExpanded.state, nextAction),nodeBeingExpanded);
        ...
        else if (cheapestCostFrontierNode.get(child.state) == null ? false: cheapestCostFrontierNode.get(child.state).pathCost > child.pathCost ) {
          frontier.add(child);
          cheapestCostFrontierNode.put(child.state, child);          
        }
      }
